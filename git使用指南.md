...
| **变基分支（git rebase 分支名）**              | 把你自己的支线存档，“剪切粘贴” 到官方最新主线存档的末尾      | 把你本地的提交，**重新接到远程最新代码的后面**，让历史变成**一条干净直线**，**没有分叉、没有合并痕迹**。 |
| **储藏暂存（git stash）**                      | 临时存档（打 BOSS 前先存个临时档，不想保存当前瞎试的操作）   | 把未提交的代码修改临时存起来，让工作区回到干净状态，后续可恢复 |
| **添加远程仓库（git remote add origin 地址）** | 绑定本地存档到官方云端（告诉本地存档：“你的云端同步地址是这个”） | 把本地仓库和远程仓库关联，后续才能推送 / 拉取                |
...
| **标签（git tag）**                            | 给重要存档打永久版本戳（v1.0）                               | 标记发布版本、稳定节点，永远不动                             |
...
**共同作用**：你的新功能代码都会合并进去，最终运行的程序完全一样功能、bug、逻辑没有任何区别

**区别**：

| 特性               | 变基（rebase）                     | 合并（merge）                      |
| ------------------ | ---------------------------------- | ---------------------------------- |
| **游戏比喻**       | 把你的支线**剪切粘贴**到主线最后   | 把支线和主线**拼合**，留一个交汇点 |
| **提交历史**       | **一条直线，非常干净**             | **有分叉，能看到合并痕迹**         |
| **是否生成新提交** | 不会生成合并提交                   | 会生成一个**合并提交**             |
| **适合场景**       | **自己的分支、同步主线**           | **公共分支、多人协作最终合并**     |
| **危险点**         | **不能用于已经推给别人的公共分支** | 安全，任何分支都能用               |

你可以点击下面跳转到对应部分，观看他们在Vs Code 图形化界面中的区别

[跳转到合并分支](#合并分支)                                                                                           [跳转到变基分支](#变基分支)
...
简单来说，你开一条新的分支，如果决定rebase，那么每次main分支有更新你就要采用如下步骤（图形化界面操作就不再重复了）：
...
```bash
git fetch origin       # 第一步：必须先拉取远程最新信息
git merge origin/main  # 第二步：再合并最新的远程main
```
...
1.公共主分支 main  等，意思是，你不能把main rebase到其它分支，但是当main分支更新的时候，你可以将main分支更新后的内容merge 到你开发的功能分支。

2.多人共用同一条开发分支：比如我要和其他人共同开发 feature/motor 分支，

3**.想保留分叉痕迹，保留分支开发的记录**。

**总之rebase就是重要的分支（main）不能rebase 到其它分支，但是其它分支可以rebase 到重要分支（main）中**，但是这样会没有明显的分支开发痕迹。（相当于上面那张图片把黄色的线全部去掉，这里有黄色线是因为我之前该分支merge到了main分支，当时没有rebase，这个分叉会保留）

****

#### 5.克隆仓库

克隆仓库可以http和ssh两种形式这两种形式没有太大的区别

| 方式      | 登录方式        | 要不要输密码       | 难度       |
| --------- | --------------- | ------------------ | ---------- |
| **HTTPS** | 账号密码 / 令牌 | 每次都要           | 最简单     |
| **SSH**   | 密钥（钥匙）    | 一次配置，永久免密 | 稍配置一次 |

 我将用命令行和Vs Code 两种方式展现如何克隆
...
Git 的rebase是「复制提交 + 重新应用」，不是「移动提交」：
第一次合并到 main 的提交，已经成为 main 历史的一部分
你在旧分支再次 rebase 时，Git 不认为 “这些提交已经合并过”，只会无脑复制一遍
最终结果：同一个改动，出现两个不同 ID 的提交，图表里就显示重复。

**因此我们需要合并 pr 后，立刻删除旧分支（本地 + 远程），你要确保你的pr必须是这个分支开发的最终版本**！
...
常见的是这些，一般多人协作时会统一规定，我自己开发的时候并不是特别注意，如果你觉得懒得记，可以在VsCode上下载拓展，可以帮助你写，我就不展示了。
